...
const processResult = (result: any): VulResult[] => {

	var results: VulResult[] = [];
	...
	if (result.Misconfigurations) {
		for (let i = 0; i < result.Misconfigurations.length; i++) {
			...
			const vulResult = new VulResult(element.ID,
				element.Title,
				element.Description,
				result.Target,
				startLine,
	...
	if (result.Vulnerabilities) {
		...
		for (let i = 0; i < result.Vulnerabilities.length; i++) {
			...
			const vulResult = new VulResult(element.VulnerabilityID,
				element.Title,
				element.Description,
				result.Target,
				1, 1,
	...
	if (result.Secrets) {
		...
		for (let i = 0; i < result.Secrets.length; i++) {
			...
			const vulResult = new VulResult(element.RuleID,
				element.Title,
				element.Description,
				result.Target,
				element.StartLine, 

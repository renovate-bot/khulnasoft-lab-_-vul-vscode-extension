...
import * as vulExtension from '../../extension';

suite('Extension Test Suite', () => {
	vscode.window.showInformationMessage('Start all tests.');
	
	test('Not a vulnerable project', () => {
		var got = vulExtension.runCommand("vul --quiet filesystem --exit-code=10", "src/test/golden/not-vulnerable");
		assert.strictEqual(got, ``, "should be equal and no vulnerabilities found");
	});

	test('Vulnerable project', () => {
		var got = vulExtension.runCommand("vul --quiet filesystem --exit-code=10", "src/test/golden/vulnerable");
